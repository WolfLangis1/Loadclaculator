...
import { TableCell } from '../../../components/UI/ResponsiveTable';
...
export const LoadRow: React.FC<LoadRowProps> = React.memo(
  ({ load, updateLoad, removeLoadRow, validateField, isMobile, toggleRowExpansion, isExpanded }) => {
    ...
    return (
      <tr key={load.id} className="hover:bg-gray-50">
        ...
        <TableCell className="load-volts-cell">
          <select
            value={load.volts || 120}
            onChange={(e) => handleUpdate('volts', parseInt(e.target.value))}
            className="w-full text-center border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
          >
        ...
        <TableCell className="load-va-cell">
          <span className="text-sm font-mono text-gray-700">
            {load.va?.toLocaleString() || '0'}
          </span>
        </TableCell>
        <TableCell className="load-total-cell">
          <span className="text-sm font-mono font-medium text-gray-900">
            {load.total?.toLocaleString() || '0'}
          </span>
        </TableCell>
        <TableCell className="load-critical-cell">
        ...
        <TableCell className="load-actions-cell">
          <div className="flex justify-center">
            <button
              onClick={() => removeLoadRow(load.id)}
